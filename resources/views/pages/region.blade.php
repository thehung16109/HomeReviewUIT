...
@section('content')
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><a class="text-light"
                            style="font-weight:bolder">{{ $region_name }}</a></li>
                </ol>
            </nav>
        </div>
    </section>
    ...
    <section class="homestay mt-5">
        <div class="container p-0">
            <h1 class="mt-5">{{ $region_name }}</h1>
            <div class="row mt-5">
                <div class="col-md-8 col-lg-8 col-xl-8 homestay-left ">
                    @foreach ($all_review_region as $key => $review_region)
                        <div class="row mb-4 wow bounceInLeft hs-item">
                            <div class="col-md-5 col-lg-5 col-xl-5 left-img">
                                <a href="{{ URL::to('/review/' . $review_region->review_slug) }}">
                           
                                        <img class="myimg"
                                            src="../uploads/ReviewImage/{{ explode('|', $review_region->review_images)[0] }}">
                           
                                    <div class="homestay-overlay">
                                        <a class="location-tag"
                                            href="{{ URL::to('/location/' . $review_region->location_slug) }}">
                                            <h6>{{ $review_region->location_name }}</h6>
                                        </a>
                                    </div>
                                    <div class="homestay-overlay-3">
                                        <a class="location-tag"
                                            href="{{ URL::to('/category/' . $review_region->category_slug) }}">
                                            <h6>{{ $review_region->category_name }}</h6>
                                        </a>
                                    </div>
                                </a>
                            </div>
                            <div class="col-md-7 col-lg-7 col-xl-7">
                                <a href="">
                                    <a class="review-title"
                                        href="{{ URL::to('/review/' . $review_region->review_slug) }}">
                                        <h4>{{ $review_region->review_title }}</h4>
                                    </a>
                                </a>
                                <p>{{ $review_region->review_desc }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
